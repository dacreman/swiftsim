...
def plot_photons(filename, energy_boundaries = None, flux_boundaries = None):
    ...
    if plot_analytical_solutions:
        ...
        negatives = advected_positions < 0.
        if negatives.any():
            while advected_positions.min() < 0.:
        ...
        analytical_solutions = np.zeros((part_positions.shape[0], ngroups), dtype=np.float)
    ...
    if plot_all_data:
        ...
        for g in range(ngroups):
            ...
            if plot_analytical_solutions:
                ax.plot(part_positions[s], analytical_solutions[s,g], **analytical_solution_kwargs, label='analytical solution')
            ax.scatter(part_positions, photon_energy, **scatterplot_kwargs, label='simulation')
            ax.legend()

            ax.set_title("Group {0:2d}".format(g+1))
            if g == 0:
                ax.set_ylabel("Energies [$" + photon_energy.units.latex_representation() + "$]")
